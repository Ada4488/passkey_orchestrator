 ...
 */
function detectPasskeyProviders(os, browser) {
    const providers = [];
...
export async function detectEnvironment() {
    try {
        ...
        throw new PasskeyEnvironmentError('Failed to detect environment capabilities', 'detection_failed', error);
    }
}
/**
...
export function validateEnvironment(environment) {
    if (!environment.isWebAuthnSupported) {
        throw new PasskeyEnvironmentError('WebAuthn is not supported in this environment', 'webauthn_not_supported');
    }
    // Check for secure context
    if (typeof window !== 'undefined' && !window.isSecureContext) {
        throw new PasskeyEnvironmentError('WebAuthn requires a secure context (HTTPS)', 'insecure_context');
    }
}
/**
 ...
 */
export function detectPasskeyProvider() {
    const os = detectOS();
    const browser = detectBrowser();
