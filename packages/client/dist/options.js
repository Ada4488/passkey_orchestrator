 ...
 */
import { generateChallenge, base64URLToArrayBuffer, stringToArrayBuffer, } from '@passkey-orchestrator/shared';
/**
 ...
 */
export async function getRecommendedPasskeyOptions(userId, username, environment, config) {
    ...
    const authenticatorAttachment = getRecommendedAuthenticatorAttachment(environment, config.preferredAuthenticatorType);
 ...
 */
function getRecommendedAuthenticatorAttachment(environment, preference) {
    ...
    const highConfidenceCrossPlatform = detectedProviders.some(p => p.type === 'cross-platform' && p.confidence > 0.7);
 ...
 */
function getRecommendedAttestation(environment) {
    ...
    const hasEnterpriseProviders = detectedProviders.some(p => p.name.includes('Windows Hello') || p.name.includes('Microsoft'));
 ...
 */
export function getPasskeySuggestions(environment) {
    const suggestions = [];
 ...
 */
export function getAuthenticationInstructions(environment) {
    const instructions = [];
    const { os, isPlatformAuthenticatorAvailable, detectedProviders } = environment;
    if (isPlatformAuthenticatorAvailable) {
        switch (os) {
 ...
 */
export function getEnvironmentSpecificOptions(environment) {
    return {
 ...
 */
export async function generateRegistrationOptions(userId, username, displayName) {
    const challenge = base64URLToArrayBuffer(generateChallenge());
    const userIdBuffer = stringToArrayBuffer(userId);
    return {
        challenge,
 ...
 */
function getBrowserSpecificHints(browser) {
    switch (browser) {
        case 'Safari':
            return {
